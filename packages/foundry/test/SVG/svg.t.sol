...
// check SVG
// check removal of ogImage
// check toggle off
// check add accessory

// Try all tests with ERC1155 as OG image

// mint accessory and check SVG
// mint with wrong address
// fuzz test size, x, y
// multiple accessories vs. one accessory

//forge test -v --match-path test/SVG/svg.t.sol --via-ir
contract svg is Test {
    ...
    //build the SVG test with a simple example, then you can fuzz all parameters.

    // Tests a single OG Image with a Single Accessory
    // mintWallets[0] = nft id 0
    function testOneSVG() public {
        ...
        assertEq(degen.balanceOf(mintWallets[0]), mintStartDegenBalance - 100);
        assertEq(degen.balanceOf(owner), beneficiaryStartDegenBalance + 50);
        assertEq(degen.balanceOf(accessoryWallets[0]), royatyStartDegenBalance + 50);

        string memory basedDerivURI = based.tokenURI(0);
        string memory getSvg = getFinalSVG(basedDerivURI);
        vm.writeFile("./test/SVG/outputs/based.svg", string(getSvg));
        _testEditBasedDerivative(0);
        _testToggleBasedDerivative(1, 0, 0, false);
        _testToggleBasedDerivative(2, 0, 0, true);
        _testRemoveOgImage(0);
    }

    function _testRemoveOgImage(uint256 tokenId) internal {
        vm.startPrank(mintWallets[0]);
        tnng.transferFrom(address(mintWallets[0]), address(owner), tokenId);
        vm.stopPrank();

        string memory basedDerivURI = based.tokenURI(tokenId);
        string memory getSvg = getFinalSVG(basedDerivURI);
        vm.writeFile("./test/SVG/outputs/basedNoOG.svg", string(getSvg));
    }

    function _testToggleBasedDerivative(uint256 fileName, uint256 tokenId, uint256 _accessoryId, bool _isVisible)
        internal
    {
        string memory previewImageURL = "test 1";

        vm.startPrank(mintWallets[0]);
        based.toggleAccessoryOnOff(tokenId, _accessoryId, _isVisible, previewImageURL);
        vm.stopPrank();

        string memory basedDerivURI = based.tokenURI(tokenId);
        string memory getSvg = getFinalSVG(basedDerivURI);
        string memory filename =
            string(abi.encodePacked("./test/SVG/outputs/basedOnOff", SVGBuilder._toString(fileName), ".svg"));

        vm.writeFile(filename, string(getSvg));
    }

    function _testEditBasedDerivative(uint256 tokenId) internal {
        SVGBuilder.Accessory memory accessory1 = _buildAccessory(
            0, // accessoryId
            "https://ipfs.io/ipfs/QmYs8NVxVdHsxGwA5g1vjzfZKQnHGwdHkf2SN97rVGyMtc/422.png", // imageURL
            1000, // height
        ...
        SVGBuilder.Accessory[] memory _accessories = new SVGBuilder.Accessory[](1);
        _accessories[0] = accessory1;
        string memory previewImageURL = "test 3";

        vm.startPrank(mintWallets[0]);
        based.editBasedDerivative(tokenId, _accessories, previewImageURL);
        vm.stopPrank();

        //  SVGBuilder.TokenData storage tokenDataInstance = based.tokenData(tokenId);

        string memory basedDerivURI = based.tokenURI(tokenId);
        string memory getSvg = getFinalSVG(basedDerivURI);
        vm.writeFile("./test/SVG/outputs/basedEdit.svg", string(getSvg));
    }

    function _tstEditBasedDerivativeInvalidId(uint256 tokenId) internal {
        string memory previewImageURL = "test 2";
    ...
    function _checkMintBalanceUpdates() internal {}
    ...
    function _mintBasedDerivative(SVGBuilder.ogImageData memory _ogImage, SVGBuilder.Accessory[] memory _accessories)
        internal
    {
        string memory previewImageURL = "test 4";
    ...
    function _addAllAccessories(uint256 _amount) internal {
        ...
        string memory trait = "Hat";
        string[] memory value = _generateValues();
        uint256 mintingFee = 100;
        uint256 gateAmount = 1000;
        uint256 amoutnPerTokenId = 1;
        uint16 ercGateType = 20;
        uint16 royalty = 5000;
        string memory imageURL = "https://ipfs.io/ipfs/QmYs8NVxVdHsxGwA5g1vjzfZKQnHGwdHkf2SN97rVGyMtc/422.png";
        SVGBuilder.AccessoryCallData memory data;
        SVGBuilder.AccessoryMintingParams memory params;

        for (uint256 i; i < _amount; i++) {
            data = _createAccessoryCallDataStruct(i, totalSupply, 0, amoutnPerTokenId, trait, value[i]);
            params = _createAccessoryMintingParamsStruct(
                address(accessories), i, mintingFee, gateAmount, address(degen), address(degen), ercGateType, royalty
            );

            _addAccessory(imageURL, data, params);
    ...
    function _checkAllStructs(
        ...
        (
            ...
            string memory value
        ) = based.accessoryData(accessoryId);
        ...
        (
            ...
            uint16 ercGateType,
            uint16 royalty
        ) = based.accessoryMintingParams(accessoryId);

        // Comparing AccessoryData fields
        assertEq(_data.accessoryId, accessoryIdReturned);
        assertEq(_data.totalSupply, totalSupply);
        assertEq(_data.amountMinted, amountMinted);
        assertEq(_data.amountPerTokenId, amountPerTokenId);
        assertEq(_data.traitType, traitType);
        assertEq(_data.value, value);

        // Comparing AccessoryMintingParams fields
        assertEq(_params.accessoryNFTCollection, accessoryNFTCollection);
        assertEq(_params.tokenId, tokenId);
        assertEq(_params.mintingFee, mintingFee);
        assertEq(_params.gateAmount, gateAmount);
        assertEq(_params.feeAddress, feeAddress);
        assertEq(_params.gateAddress, gateAddress);
        assertEq(_params.ercGateType, ercGateType);
        assertEq(_params.royalty, royalty);
    }
    ...
    function _generateAccessoryWallets(uint16 amount) internal {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            console.log("accessory address");
            console.log(accessoryWalletAddress);
    ...
    function getFinalSVG(string memory base64TokenURI) public pure returns (string memory) {
        string memory b64tokenURIfixed = removeBase64Prefix(base64TokenURI);
        bytes memory miladySvg = Base64s.decode(b64tokenURIfixed);
        string memory finalSVG = extractImageBase64(string(miladySvg));
        bytes memory finalSVGbytes = Base64s.decode(finalSVG);
        return string(finalSVGbytes);
    }

    function extractImageBase64(string memory metadata) public pure returns (string memory) {
        // Search for the substring "base64," in the input metadata
        bytes memory metadataBytes = bytes(metadata);
        bytes memory base64Identifier = bytes("base64,");
        uint256 start = 0;

        for (uint256 i = 0; i < metadataBytes.length - base64Identifier.length; i++) {
            bool found = true;
            for (uint256 j = 0; j < base64Identifier.length; j++) {
                if (metadataBytes[i + j] != base64Identifier[j]) {
                    found = false;
                    break;
                }
            }
            if (found) {
                start = i + base64Identifier.length;
                break;
            }
        }

        // Extract and return the Base64 encoded string
        bytes memory base64Encoded = new bytes(metadataBytes.length - start);
        for (uint256 i = start; i < metadataBytes.length; i++) {
            base64Encoded[i - start] = metadataBytes[i];
        }
        return string(base64Encoded);
    }

    function removeBase64Prefix(string memory tokenURI) public pure returns (string memory) {
        string memory prefix = "data:application/json;base64,";
        uint256 prefixLength = bytes(prefix).length;
        return substring(tokenURI, prefixLength);
    }

    function substring(string memory str, uint256 startIndex) public pure returns (string memory) {
        bytes memory strBytes = bytes(str);
        uint256 resultLength = strBytes.length - startIndex;
